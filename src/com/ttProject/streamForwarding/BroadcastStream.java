...
public class BroadcastStream implements IBroadcastStream, IProvider, IPipeConnectionListener
{
	...
	public void terminateGhostConnection() {
		if(mLivePipe.getConsumers().size() != 0) {
			// we still have some users. leave the connection to save stream.
...
//		this.mLivePipe.subscribe(getProvider(), new HashMap<String, Object>() {{put("play", "start");}});
	...
	public void stop()
	{
		// send unscribe. we can stop, but no start.
		...
		try {
			...
			if (event instanceof IRTMPEvent)
			{
				...
				if (mLivePipe != null)
				{
					...
					//RTMPMessage msg = new RTMPMessage(); // older version for making RTMPMessage instance.
					...
					try
					{
						if (event instanceof AudioData)
						{
							...
							// we need to get audio data, however red5 doesn't have AudioCodecFactory now.
						}
